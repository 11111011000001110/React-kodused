...
function Products() {
  ...
  const [sortPriceDirection, setPriceSortDirection] = useState(0);
  const [sortNameDirection, setNameSortDirection] = useState(0);
  ...
  const sortByPrice=()=>{
    const sorted = products.sort((a,b)=>{
      if(a.price>b.price)return (sortPriceDirection||1)*1;
      if(a.price<b.price)return (sortPriceDirection||1)*-1;
      return 0;
    })
    setNameSortDirection(0);
    setPriceSortDirection((sortPriceDirection||1)*-1);
    setProducts(sorted);
  }
  const sortByName=()=>{
    const sorted = products.sort((a,b)=>{
      ...
      return aName.localeCompare(bName)*(sortNameDirection||1);
    })
    setPriceSortDirection(0);
    setNameSortDirection((sortNameDirection||1)*-1);
    setProducts(sorted);
  }
  return (<div>
    <div className="container">
      ...
      <Table className="table table-hover table-bordered">
        <thead>
        <tr>
          <th scope="col" onClick={sortByName} className="cursor-pointer">Product {sortNameDirection===1?<Icon.ArrowUp/>:sortNameDirection===-1?<Icon.ArrowDown />:null}</th>
          <th scope="col" onClick={sortByPrice} className="cursor-pointer">Price (€) {sortPriceDirection===1?<Icon.ArrowUp/>:sortPriceDirection===-1?<Icon.ArrowDown />:null}</th>
        ...
        {products.map(product => 
            <tr key={product.name + product.price}>
              ...
              <td className={product.quantity<3?'text-danger':null}>{product.quantity}</td> 
              <td>{product.store}</td>
            </tr>
          )}
